...
const RecentCard = () => {
  ...
  const [likedTurfs, setLikedTurfs] = useState(new Set()); // State for liked turfs
  ...
  const handleLikeToggle = (event, turfId) => {
    event.preventDefault(); // Prevents navigation
    setLikedTurfs(prevLikedTurfs => {
      const newLikedTurfs = new Set(prevLikedTurfs);
      if (newLikedTurfs.has(turfId)) {
        newLikedTurfs.delete(turfId);
      } else {
        newLikedTurfs.add(turfId);
      }
      return newLikedTurfs;
    });
  };
      ...
      <Grid container alignItems="center" style={{ minHeight: '10vh' }}>
        ...
        <Grid item xs={12} sm={12} md={2}>
          <Typography variant="body1">
            <button className='btn3' style={{ backgroundColor: "#0fb719 " }}>   {filteredTurfs.length} Turfs Found </button>
          </Typography>
        </Grid>
      </Grid>
                ...
                <Link to={`/turf/${turfId}`} style={{ textDecoration: 'none' }}>
                  ...
                  <CardContent>
                    <div className='category flex'>
                      ...
                      <IconButton onClick={(e) => handleLikeToggle(e, turfId)}>
                        <FavoriteIcon style={{ color: likedTurfs.has(turfId) ? 'red' : 'inherit' }} />
                      </IconButton>
                    </div>
                    <Typography variant="h6" color="#0fb719">{name}</Typography>
                    <Typography variant="body2">
                      <i className='fa fa-location-dot'></i> {address}
                    </Typography>
                    <div className='button flex'>
                      <Typography variant="body1">
                        <button className='btn2' style={{ backgroundColor: "#0fb719" }}>{price}</button> <label>/hr</label>
                      </Typography>
